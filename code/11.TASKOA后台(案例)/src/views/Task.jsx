...
class Task extends React.Component {
    ...
    columns = [{
        ...
        align: 'center',
        width: '10%',
        // +text：将字符串 text 强制转换为数字（例如 "1" → 1，"abc" → NaN）。
        ...
        align: 'center',
        width: '15%',
        //render 有3个参数：text、record、index
        //text: 当前单元格的文本内容
        //record: 当前行的数据对象，包含该行所有列的数据
        //index: 当前行的索引值
        render: (_, record) => {
    ...
    state = {
        tableData: [
            { id: 1, task: '任务1', state: 1, time: '2023-10-01 12:00', complete: '2023-10-02 12:00' },
            { id: 2, task: '任务2', state: 2, time: '2023-10-01 12:00', complete: '2023-10-02 12:00' },
        ],
        tableLoading: false,
        modalVisible: false,
    ...
    changeIndex = (index) => {
        ...
        // 由于setState是异步的，所以在setState后面直接调用queryData()，可能会导致selectedIndex的值不正确
        /*方法一 是用setState 的回调
        this.setState({
            selectedIndex: index
        }, () => {
    ...
    render() {
        ...
        return <div className="task-box">
            ...
            <Modal title="新增任务窗口" open={modalVisible} confirmLoading={confirmLoading} keyboard={false} maskClosable={false} okText="确认提交" onCancel={this.closeModal} onOk={this.submit}>
                {/* antd中的表单自动收集输入的信息，通过item中的name字段，默认是在内容onChange事件中，也可以基于tigger改变收集的时机 */}
