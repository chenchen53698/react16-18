...
const Detail = function Detail(props) {
    ...
    const handleImage = (result) => {
        ...
        // 创建大图 这是 document.createElement('img') 的简写形式，专门用于创建图像元素。
        let tempImg = new Image;
        tempImg.src = result.image;
        tempImg.onload = () => {//设置加载完成事件回调
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // console.log(result)
                //document.querySelector('.img-place-holder');这个属性在handleImage(result);被使用，
                //它只有在视图渲染后才能拿到
                flushSync(() => {
                    setInfo(result);
                    handleStyle(result);
    ...
    useEffect(() => {
        (async () => {
            // 第一次渲染完:如果userInfo不存在,我们派发任务同步登录者信息（+保险）
            if (!userInfo) {
    ...
    return <div className="detail-box">
        {/* 新闻内容 */}
        {!info ? <SkeletonAgain /> :
            // dangerouslySetInnerHTML 是 React 中的一个特殊属性，用于直接设置元素的 HTML 内容。它提供了一种在 React 组件中插入原始 HTML 的方式
